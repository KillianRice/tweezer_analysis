function [avgDataset] = param_ext_CloudRadius_avg(analyVar,avgDataset)
...
for basenameNum = 1:analyVar.numBasenamesAtom
    ...
    [cldRadX cldRadY]...
        = deal(NaN(sum(analyVar.LatticeAxesFit),avgDataset.CounterAtom));
...
%%%%%%%-----------------------------------%%%%%%%%%%
        % Size of cloud
        cldRadX(:,k) = cellfun(@(x) funcCloudRad(x,paramRadX),avgDataset.All_fitParams{k});
        cldRadY(:,k) = cellfun(@(x) funcCloudRad(x,paramRadY),avgDataset.All_fitParams{k});
    end
    
    % Save cloud radius for each window into the indivDataset structure
    avgDataset.cloudRadX = cldRadX;
    avgDataset.cloudRadY = cldRadY;
end