function output = MCS_Cum_SFI(analyVar, indivDataset, avgDataset)
    ...
    for i = 1:analyVar.numBasenamesAtom  %Scan over all the file names selected in batch files.
        ...
        indivDataset{i}.cumSFI(:,1) = indivDataset{i}.mcsSpectra{1}(:,1);       % Place the x values
        totsfi = zeros(size(indivDataset{i}.mcsSpectra{1}(:,2:end)));           % Dummy list that will add up all the y values for each corresponding x
    ...
    for id = 1:length(scanIDs)
        x{id} = indivDataset{1}.mcsSpectra{1}(:,1);
