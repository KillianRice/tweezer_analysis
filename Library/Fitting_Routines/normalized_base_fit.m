function [xav,yav,yer,coefflist,coefflist_err] = normalized_base_fit(analyVar, indivDataset, avgDataset, form, indVarField, depVarField, x0, useroptions)
    ...
    if plotIndivFits
        ...
        for i = 1:analyVar.numBasenamesAtom
            ...
            %% Addition for getting max value for normalization
            [maxvalue,offset] = max(ydata{i});
            ydata{i} = ydata{i}/maxvalue;
    ...
    if length(analyVar.timevectorAtom) > 1 && plotAvgFits
        ...
        for i = 1:length(scanIDs)
            ...
            %% Addition for getting max value for normalization
            [maxvalue,offset] = max(yavg{i});
            yavg{i} = yavg{i}/maxvalue;
            yerr{i} = yerr{i}/maxvalue;
